...
import { AddToAppealsModal } from './AddToAppealsModal';
...
export function RecentInvoices({ isLoading, invoices, onRefresh }: RecentInvoicesProps) {
  ...
  const [appealModal, setAppealModal] = useState<Invoice | null>(null);
  ...
  const handleDownloadPDF = async (invoice: Invoice) => {
    ...
    try {
      ...
      if (result.method === 'ios-share') {
        ...
        setToastMessage(`✅ PDF for ${invoice.invoiceNumber} downloaded!`);
      }
  ...
  const handleAddToAppeals = (invoice: Invoice) => {
    setAppealModal(invoice);
    setOpenDropdown(null);
  };
  ...
  return (
    <>
                                ...
                                <div
                                  ...
                                  <div className="py-1">
                                    ...
                                    <div className="border-t border-gray-100 mt-1 pt-1">
                                      ...
                                      <button
                                        onClick={() => handleAddToAppeals(invoice)}
                                        className="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-gray-50 flex items-center space-x-2"
                                      >
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="border-t border-gray-200 pt-2">
                  ...
                  <button
                    onClick={() => {
                      const invoice = sortedInvoices.find(inv => inv.id === openDropdown);
                      if (invoice) handleAddToAppeals(invoice);
                    }}
                    className="w-full text-left px-4 py-3 text-base text-gray-700 hover:bg-gray-50 active:bg-gray-100 flex items-center space-x-3 rounded-lg transition-colors"
                  >
      ...
      {appealModal && (
        <AddToAppealsModal
          prefill={{
            invoiceRef: appealModal.invoiceNumber,
            requesterName: appealModal.manualClient?.name || appealModal.manualClient?.company || '',
            trialCourtCaseNumber: appealModal.customFields?.causeNumber || '',
            courtOfAppealsNumber: appealModal.customFields?.coaNum || '',
            style: appealModal.customFields?.caseName || '',
          }}
